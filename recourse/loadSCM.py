...
def loadSCM(scm_class, experiment_folder_name = None):
  ...
  if scm_class == 'sanity-3-lin':
  ...
  elif scm_class == "diamond":
    structural_equations_np = {
      'x1': lambda n_samples,        : n_samples,
      'x2': lambda n_samples, x1     : x1*x1 + n_samples,
      'x3': lambda n_samples, x1     : 4*np.sqrt(np.abs(x1)) + n_samples,
      'x4': lambda n_samples, x2, x3 : 2*np.sin(x2)+2*np.sin(x3) + n_samples,
    }
    structural_equations_ts = {
      'x1': lambda n_samples,        : n_samples,
      'x2': lambda n_samples, x1     : x1*x1 + n_samples,
      'x3': lambda n_samples, x1     : 4*torch.sqrt(torch.abs(x1)) + n_samples,
      'x4': lambda n_samples, x2, x3 : 2*torch.sin(x2)+2*torch.sin(x3) + n_samples,
    }
    noises_distributions = {
      'u1': Uniform(-5, 5),
      'u2': Normal(0, 1),
  ...
  elif scm_class == "lin4v":
    structural_equations_np = {
      'x1': lambda n_samples,        : n_samples,
      'x2': lambda n_samples, x1     : 4*x1 + n_samples,
      'x3': lambda n_samples, x1     : 16*np.sqrt(np.abs(x1)) + n_samples,
      'x4': lambda n_samples, x3     : 2*np.log(np.abs(x3)) + n_samples,
    }
    structural_equations_ts = {
      'x1': lambda n_samples,        : n_samples,
      'x2': lambda n_samples, x1     : 4*x1 + n_samples,
      'x3': lambda n_samples, x1     : 16*torch.sqrt(torch.abs(x1)) + n_samples,
      'x4': lambda n_samples, x3     : 2*torch.log(torch.abs(x3)) + n_samples,
    }
    noises_distributions = {
      'u1': Uniform(-5, 5),
      'u2': Normal(0, 1),
    ...
    structural_equations_np = {
      ...
      'x3': lambda n_samples, x1, x2 : b + b0 * np.sign(n_samples) * (b1 * x1**2 + b2 * x2**2) + n_samples,
    }
    structural_equations_ts = {
      'x1': lambda n_samples,        :                                   n_samples,
      'x2': lambda n_samples, x1     :      a0 * torch.sign(torch.tensor(n_samples)) * (x1 ** 2) * (1 + n_samples**2),
      'x3': lambda n_samples, x1, x2 : b + b0 * torch.sign(torch.tensor(n_samples)) * (b1 * x1**2 + b2 * x2**2) + n_samples,
    }
    noises_distributions = {
    ...
    structural_equations_np = {
      ...
      'x2': lambda n_samples, x1        :                 0.5 * x1 + n_samples,  # A --> X_1
      'x3': lambda n_samples,           :                            n_samples,  # X_2
      'x4': lambda n_samples, x1, x2, x3    : 0.5 * (x1 + x2 - x3) + n_samples,  # {X_1, X_2} --> X_3
    }
    structural_equations_ts = structural_equations_np
    noises_distributions = {
      'u1': Bernoulli(0.5, '-11'),
    ...
    structural_equations_np = {
      ...
      'x2': lambda n_samples, x1:                                  0.5 * x1 + n_samples,  # A --> X_1
      'x3': lambda n_samples,:                                                n_samples,  # X_2
      'x4': lambda n_samples, x1, x2, x3:  0.5 * x1 + 0.1 * (x2**3 - x3**3) + n_samples,  # {X_1, X_2} --> X_3
    }
    structural_equations_ts = structural_equations_np
    noises_distributions = {
      'u1': Bernoulli(0.5, '-11'),
